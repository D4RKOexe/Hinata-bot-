...
let handler = async (m, { conn }) => {
  ...
  if (!user) {
    global.db.data.users[who] = { diamantes: 0, darkcoins: 0, bank: 0, exp: 0, level: 0 }
  ...
  if (!victim) {
    global.db.data.users[target] = { darkcoins: 0, darkcoins: 0, bank: 0 }
    victim = global.db.data.users[target]
  }

  let victimDarkcoins = victim.darkcoins || victim.diamond || 0

  if (victimDiamantes <= 0) {
    return conn.sendMessage(m.chat, {
      text: '🥷 「 HINATA STEAL 」 🥷\n✦•┈๑⋅⋯ ⋯⋅๑┈•✦\n\n💫 » @' + target.split('@')[0] + ' no tiene darkcoins\n\n✦•┈๑⋅⋯ ⋯⋅๑┈•✦',
      mentions: [target]
    }, { quoted: m })
  }
  ...
  if (Math.random() < 0.55) {
    let maxRobo = Math.floor(victimDarkcoins * 0.6)
    let robado = Math.floor(Math.random() * maxRobo) + 1

    if (victim.darkcoins !== undefined) {
      victim.darkcoins = victimDiamantes - robado
    } else {
      victim.darkcoins = victimDarkcoins - robado
    }

    user.darkcoins = (user.darkcoins || user.darcoin || 0) + robado
    ...
    await conn.sendMessage(m.chat, {
      text: '🥷 「 DARK STEAL 」 🥷\n✦•┈๑⋅⋯ ⋯⋅๑┈•✦\n\n💫 » ROBO EXITOSO\n\n🎯 » @' + target.split('@')[0] + '\n💵 » Robaste: ' + robado + ' darrkcoins\n💰 » Tu total: ' + (user.darkcoins || user.darkcoin || 0) + ' 💎\n\n' + mensajes[Math.floor(Math.random() * mensajes.length)] + '\n\n✦•┈๑⋅⋯ ⋯⋅๑┈•✦\n> ⏳ 45 minutos',
      mentions: [target]
    }, { quoted: m })
  } else {
    let multa = Math.floor(Math.random() * 8) + 3
    let misDarkcoins = user.darkcoins || user.diamond || 0
    misDarkcoins = Math.max(0, misDarkcoins - multa)
    if (user.darkoinss !== undefined) {
...
handler.desc = 'Roba darkcoins a otros usuarios'
