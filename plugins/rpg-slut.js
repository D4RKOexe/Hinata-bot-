...
let handler = async (m, { conn }) => {
  ...
  let darkcoins, personas, exp, tipo

  if (random < 0.04) {
    personas = Math.floor(Math.random() * 6) + 5
    darkcoins = personas * 2
    ...
    darkcoins = Math.floor(personas * 1.5)
    ...
    darkcoins = Math.floor(personas * 0.5) || 1
  ...
  user.diamantes = Math.max(0, (user.darkcoins || 0) + darkcoins)
  ...
  texto += '💵 » Darkcoins: ' + (darkcoins > 0 ? '+' : '') + darkcoins + '\n'
